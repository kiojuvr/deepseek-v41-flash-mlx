...
def main():
 ...
 with torch.inference_mode():
  for t in range(n):
   ...
   for _,contribution in sorted(contributions,key=lambda item:item[0]): routed_y=routed_y+contribution.to(torch.bfloat16)
   y=shared_y.to(torch.bfloat16).float()+routed_y
