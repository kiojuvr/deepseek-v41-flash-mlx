...
def main():
 ...
 with torch.inference_mode():
  for t in range(n):
   row=x[t:t+1]; shared_y=shared(row); y=shared_y; routed_y=torch.zeros_like(y); seen=[]
   for k in range(6):
    eid=int(ids[t,k]); seen.append(eid)
    if eid not in cache: cache[eid]=Expert(weights,f'layers.0.ffn.experts.{eid}')
    contribution=cache[eid](row,float(rw[t,k])); routed_y=routed_y+contribution; y=y+contribution
   shared_rows.append(shared_y.to(torch.bfloat16)); routed_rows.append(routed_y.to(torch.bfloat16)); outputs.append(y.to(torch.bfloat16)); used.append(seen)
   if (t+1)%8==0: print(f'Completed token {t+1}/{n}',flush=True)
 result=torch.cat(outputs).contiguous(); shared_result=torch.cat(shared_rows); routed_result=torch.cat(routed_rows); nr=torch.from_numpy(native.view(np.uint16).copy()).view(torch.bfloat16)
 diff=(result.float()-nr.float()).abs(); bits=result.view(torch.int16)!=nr.view(torch.int16)
 np.save(a.output.with_suffix('.shared.bf16.npy'),shared_result.view(torch.uint16).numpy().view('|V2'),allow_pickle=False); np.save(a.output.with_suffix('.routed.bf16.npy'),routed_result.view(torch.uint16).numpy().view('|V2'),allow_pickle=False)
