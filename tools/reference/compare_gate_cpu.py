...
def main():
    ...
    report={'scope':'Official gate CPU formula vs native layer0 gate trace; no expert output or full-model qualification','raw_scores':stats(native_raw,scores),'preactivation_cpu_probe':stats(native_raw,z),'corrected_scores':stats(native_corr,corrected),'selected_id_mismatch':int((native_ids!=ids).sum()),'route_weight_cpu_max':float(route.max()),'route_weight_native_unavailable':True,'margin_min':float(margins.min()),'margin_max':float(margins.max()),'margin_below_1e-3':int((margins.abs()<1e-3).sum()),'token_ids':m['token_ids'],'native_manifest_sha256':sha(a.native/'manifest.json'),'gate_manifest_sha256':sha(a.gate/'manifest.json'),'source_sha256':weights.sources,'tensor_sha256':weights.tensor_hashes,'script_sha256':sha(Path(__file__)),'full_model_qualified':False}
    ...
    a.output.write_text(json.dumps(report,indent=2)+'\n'); print(json.dumps({k:report[k] for k in ('raw_scores','corrected_scores','selected_id_mismatch','margin_min','margin_max','margin_below_1e-3')},indent=2))
