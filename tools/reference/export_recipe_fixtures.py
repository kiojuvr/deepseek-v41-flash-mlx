...
def main():
    ...
    tokenizer_path=a.checkpoint/'tokenizer.json'; tokenizer=Tokenizer.from_file(str(tokenizer_path))
    fixtures=[]
    ...
    for name,messages,kwargs in cases:
        value=enc.encode_messages(messages,**kwargs); prompt,media=value if isinstance(value,tuple) else (value,None); fixtures.append({'name':name,'messages':messages,'kwargs':kwargs,'prompt':prompt,'token_ids':tokenizer.encode(prompt,add_special_tokens=False).ids,'media':media})
    ...
    a.output.parent.mkdir(parents=True,exist_ok=True); result={'schema_version':2,'source':str(source),'source_sha256':sha(source),'tokenizer_sha256':sha(tokenizer_path),'fixtures':fixtures,'scope':'Official checkpoint encoding.py offline fixture; not production API qualification'}; a.output.write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n'); print(json.dumps({'fixtures':len(fixtures),'source_sha256':result['source_sha256'],'tokenizer_sha256':result['tokenizer_sha256']},indent=2))
