...
printf '%s\n' \
  'scope=1 model; model-lifetime 40-layer resident atlas; one transactional 2063-token layer sweep + 1 reference decode; qualified full-width batched split-K QK' \
  'resources=allow 5-10 minutes; Unified Memory budget 340 GB; about 289 GB one-time checkpoint reads; checkpoint read-only' \
  'comparison=paired with reviewed 65.403-second layer-sweep baseline; dense QMM remains disabled; only qualified full-width QK batching is added' \
  'checks=40 model-lifetime banks; zero request bank constructions/readbacks; batched/full-width and scalar/tail QK counts; state/logits/generation and wall require review' \
...
DSV41_RUNTIME_BATCHED_SPLITK_QK=1 DSV41_RUNTIME_LAYER_SWEEP=1 \
DSV41_RUNTIME_BATCHED_DENSE_QMM=0 DSV41_RUNTIME_MLX_CACHE_LIMIT_BYTES=0 \
DSV41_CONTEXT_EXECUTION=sweep DSV41_RUNTIME_EXPERT_IO_THREADS=4 \
DSV41_RUNTIME_EXPERT_ASSIGNMENT_CHUNK=128 DSV41_RUNTIME_COMPONENT_PROFILE=0 \
DSV41_CONTEXT_WALL_BUDGET_SECONDS=1200 DSV41_CONTEXT_PROJECTED_WALL_LIMIT_SECONDS=0 \
CACHE_CONDITION=unknown RUN_CONDITIONS=resident-transactional-layer-sweep-batched-splitk-qk \
