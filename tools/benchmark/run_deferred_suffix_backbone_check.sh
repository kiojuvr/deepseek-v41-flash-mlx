...
run_dir="artifacts/prefill-gap/deferred-suffix-$(date +%Y%m%d-%H%M%S)-$$"
mkdir -p "$run_dir"
finish(){
 status=$?
 /usr/bin/vm_stat > "$run_dir/system-after.txt" 2>&1 || true
 echo "$status" > "$run_dir/exit-code.txt"
 if ((status)); then echo "FAILED: inspect $run_dir; suffix state was not published; rerun fresh."; fi
}
trap finish EXIT
checkpoint=${CHECKPOINT:-/Volumes/KIOXIA-PRO-1/models/deepseek-ai/DeepSeek-V4.1-Flash}
printf '%s\n' \
 'scope=one 2541-token full packed sweep versus encoder-private exact shrinking decoder suffix' \
 'resources=allow 10-20 minutes; up to 340 GB Unified Memory; substantial read-only checkpoint expert reads; no swap expected' \
 'gate=bit-exact final hidden/pre-mix/logits, executed route ties, all persistent state/publication/hash; exact per-layer suffix frontiers' \
 "logs=$run_dir/{test.log,resource.log,identity.txt,tracked.patch,exit-code.txt}" \
 'failure=retain logs; source request state remains unchanged; exact suffix stays disconnected from production' \
 'resume=unsupported because publication is transactional; rerun this script with fresh model/request state' | tee "$run_dir/config.txt"
{
 cmake -S . -B build-mlx
...
shasum -a 256 build-mlx/dsv41-text-backbone-test tests/attention/test_text_backbone.cpp \
 ...
 include/dsv41/deferred_decoder_plan.hpp src/model/text_backbone.cpp src/model/text_encoder.cpp \
 src/model/text_decoder.cpp src/model/compressed_block.cpp src/model/reused_block.cpp \
 src/attention/compressed_layer.cpp artifacts/checkpoint/summary.json artifacts/engram/metadata.json \
...
cmd=(env DSV41_RUNTIME_LAYER_FINITE_CHECKS=0 DSV41_RUNTIME_PACKED_EXPERT_BANK=0 \
 ...
 DSV41_CHECK_DEFERRED_SUFFIX=1 build-mlx/dsv41-text-backbone-test \
...
echo 'Completed; review exact outputs, complete decoder state, route ties, and resources before production connection.'
