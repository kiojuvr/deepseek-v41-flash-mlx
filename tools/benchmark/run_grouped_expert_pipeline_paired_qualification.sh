...
root=${DSV41_GROUPED_PIPELINE_PAIRED_DIR:-artifacts/context-ladder/grouped-expert-pipeline-paired-$(date +%Y%m%d-%H%M%S)-$$}
pairs=${DSV41_GROUPED_PIPELINE_PAIRS:-5}
if [[ ! "$pairs" =~ ^[0-9]+$ ]] || ((pairs<5));then
 echo 'DSV41_GROUPED_PIPELINE_PAIRS must be an integer of at least 5' >&2;exit 2
fi
if [[ "${DSV41_GROUPED_PIPELINE_PAIRED_DRY_RUN:-0}" == 1 ]];then
...
finish(){
 ...
 if ((status));then
  echo "FAILED: inspect $root; rerun with DSV41_GROUPED_PIPELINE_PAIRED_DIR=$root to skip verified stages."
 fi
}
trap finish EXIT
...
printf '%s\n' \
 ...
 'candidate=oMLX-derived grouped primitive owns routed gate/up, official FP8 SwiGLU roundtrip, and down GatherQMM for resident 1..8-token MoE; production default remains unchanged' \
 'checks=exact generated IDs/state/position within every pair; finite output; 40 banks; zero route/index readbacks and scalar attention; candidate records 280 grouped pipeline batches' \
 ...
 "resume=DSV41_GROUPED_PIPELINE_PAIRED_DIR=$root bash tools/benchmark/run_grouped_expert_pipeline_paired_qualification.sh" \
 | tee "$root/config.txt"
echo "Logs: $root"
...
source_sha=$(shasum -a 256 tools/benchmark/run_grouped_expert_pipeline_paired_qualification.sh \
...
run_one(){
 ...
 CACHE_CONDITION=warm-alternating RUN_CONDITIONS="grouped-expert-pipeline-paired-$label" \
...
printf 'pair\tvariant\tprefill_seconds\tfirst_seconds\tadvancing_mean_seconds\tdecode_p95_seconds\tgrouped_pipeline_batches\tpeak_bytes\n' > "$summary"
for ((pair=1;pair<=pairs;++pair));do
 for variant in baseline candidate;do
  jq -r --arg pair "$pair" --arg variant "$variant" '
   ...
   [$pair,$variant,.aggregates.prefill_seconds,.phases.decode.first_token_seconds,
    ...
    .expert_bank_io_stats.grouped_pipeline_batches,.final_memory.peak_bytes] | @tsv
