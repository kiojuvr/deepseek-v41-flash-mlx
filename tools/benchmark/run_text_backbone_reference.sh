...
mkdir -p "$run_dir"
echo "Logs: $run_dir"
cmake --build build-mlx --target dsv41-text-backbone-test -j 4 2>&1 | tee "$run_dir/build.log"
shasum -a 256 build-mlx/dsv41-text-backbone-test artifacts/checkpoint/summary.json artifacts/engram/metadata.json artifacts/engram/fixture-provenance.json > "$run_dir/identity.txt"
set +e
build-mlx/dsv41-text-backbone-test "$checkpoint" artifacts/checkpoint/summary.json artifacts/engram/metadata.json 2>&1 | tee "$run_dir/test.log"
