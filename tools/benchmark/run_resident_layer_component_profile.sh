...
printf '%s\n' \
  'scope=1 model; model-lifetime 40-layer resident atlas; 2063-token prefill + 1 decode; additive attention/MoE/post-MoE GPU-completion wall' \
  'resources=allow 5-10 minutes; Unified Memory budget 340 GB; about 289 GB one-time checkpoint reads; checkpoint read-only' \
  'measurement=adds GPU synchronization at component boundaries and perturbs the normal 127.7-second resident lazy schedule' \
  'checks=model construction exactly 40 banks; every request chunk zero bank constructions; route diagnostic readbacks zero' \
...
RUN_CONDITIONS=model-lifetime-resident-layer-component-profile \
