...
run_dir="artifacts/text-decoder/run-$(date +%Y%m%d-%H%M%S)-$$"
mkdir -p "$run_dir"
echo "Logs: $run_dir"
shasum -a 256 build-mlx/dsv41-text-decoder-test artifacts/checkpoint/summary.json > "$run_dir/identity.txt"
set +e
build-mlx/dsv41-text-decoder-test "$checkpoint" artifacts/checkpoint/summary.json 2>&1 | tee "$run_dir/test.log"
