...
printf '%s\n' \
 ...
 'measurement=decode-local attention/MoE/post-MoE GPU-completion wall; synchronization perturbs normal lazy execution, so use ratios for attribution rather than production TPT' \
...
printf 'scope\tlayer_seconds\tattention_seconds\tmoe_seconds\tpost_moe_seconds\tlayer_calls\tcomponent_calls\n' \
...
jq -r '
 ...
 ["decode",$p.layer_seconds,$p.attention_path_seconds,$p.moe_path_seconds,
  $p.post_moe_seconds,$p.layer_calls,$p.component_calls] | @tsv
