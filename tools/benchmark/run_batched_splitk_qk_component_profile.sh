...
printf '%s\n' \
  'scope=1 model; resident official layout; one transactional 2063-token layer sweep + 1 reference decode; qualified full-width batched split-K QK' \
  'resources=allow 5-10 minutes; Unified Memory budget 340 GB; about 289 GB one-time checkpoint reads; checkpoint read-only' \
  'measurement=GPU synchronization after Attention/MoE/post-MoE perturbs the normal 62.619-second candidate schedule' \
  'comparison=reviewed scalar-QK layer-sweep component profile: Attention 41.179713, MoE 17.837538, post-MoE 1.129525 seconds' \
  'checks=40 banks; zero request construction/readbacks; full-width batch and scalar-tail counts; per-layer component totals require review' \
...
CACHE_CONDITION=unknown RUN_CONDITIONS=resident-layer-sweep-batched-splitk-qk-component-profile \
