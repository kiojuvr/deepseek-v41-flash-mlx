...
task_root=${DSV41_PENDING_PAIRED_DIR:-artifacts/context-ladder/deferred-pending-paired-$(date +%Y%m%d-%H%M%S)-$$}
...
printf '%s\n' \
 ...
 'baseline=production fixed-tile/ragged/cache-bounded 4K full sweeps; candidate=same path with one private 16K encoder-only plus 12,272-row exact decoder resume' \
 ...
 "resume=DSV41_PENDING_PAIRED_DIR=$task_root bash tools/benchmark/run_deferred_pending_paired_qualification.sh; verified exit-0 children are skipped" \
...
verify_run(){
 local dir=$1 deferred=$2 expected_chunks=$3
...
run_one(){
 local label=$1 deferred=$2 chunks=$3 dir="$task_root/$1"
 if verify_run "$dir" "$deferred" "$chunks"; then echo "SKIP verified $label"; return; fi
 mkdir -p "$(dirname "$dir")"
 echo "START $label"
 DSV41_CONTEXT_RUN_DIR="$dir" CONTEXT_TOKENS=32768 TEACHER_TOKENS=4096 \
  ...
  CACHE_CONDITION=warm-alternating RUN_CONDITIONS="pending-CED-paired-$label" \
  bash tools/benchmark/run_context_32k.sh 2>&1 | tee "$task_root/$label.launch.log"
 verify_run "$dir" "$deferred" "$chunks"
}
...
if [[ "${DSV41_PENDING_PAIRED_VERIFY_ONLY:-0}" == 1 ]]; then
 ...
 for label in warmup-baseline warmup-candidate; do
  ...
  if verify_run "$task_root/$label" "$deferred" "$chunks"; then echo "VERIFIED $label"; verified=$((verified+1)); fi
 done
 for ((pair=1;pair<=pairs;pair++)); do
  for variant in baseline candidate; do
   if [[ "$variant" == baseline ]]; then deferred=0; chunks=0; else deferred=1; chunks=1; fi
   if verify_run "$task_root/pair-$pair/$variant" "$deferred" "$chunks"; then
