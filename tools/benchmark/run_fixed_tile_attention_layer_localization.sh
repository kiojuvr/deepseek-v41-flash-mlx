...
printf '%s\n' \
 ...
 'result=reports first layer/stage crossing RMS 0.002 plus layer 3/4 arithmetic attribution and the first/last layer-3 attention-core mismatch token with its selected width; final state equality is intentionally not asserted after semantic divergence; diagnostic only, not qualification or performance' \
