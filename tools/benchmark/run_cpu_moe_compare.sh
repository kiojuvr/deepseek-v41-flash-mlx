...
set -euo pipefail
cd "$(dirname "$0")/../.."
native=${1:-artifacts/logits-trace/native-20260914-145608-21879}; gate=${2:-artifacts/cpu-attention/gate-native-weights-20260914}; native_output=${3:-$native}
...
set +e
/Volumes/SDXC-512/deltafin/.venv/bin/python tools/reference/cpu_moe_compare.py --checkpoint /Volumes/KIOXIA-PRO-1/models/deepseek-ai/DeepSeek-V4.1-Flash --native "$native" --native-output "$native_output" --gate "$gate" --output "$run_dir/report.json" 2>&1 | tee "$run_dir/test.log"
