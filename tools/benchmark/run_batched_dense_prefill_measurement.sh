#!/usr/bin/env bash
set -euo pipefail
cd "$(dirname "$0")/../.."

printf '%s\n' \
  'scope=1 model; resident official layout; 2063-token batched-dense-QMM prefill + 1 decode' \
  'resources=allow 5-10 minutes; Unified Memory budget 340 GB; about 289 GB one-time checkpoint reads; checkpoint read-only' \
  'checks=optimized dense QMM opt-in; 40 model-lifetime banks; zero warm bank constructions and route/index readbacks; unchanged token/logit/state gates require separate reviewed backbone result' \
  'logs=artifacts/context-ladder/32k-run-<timestamp>-<pid>/{result.json,result.json.progress.jsonl,resource.log,identity.txt,tracked.patch}' \
  'failure=retain failed directory and inspect exit-code/test/resource logs; partial atlas/state is never reused' \
  'resume=unsupported because publication is transactional; rerun for fresh model/request state'

CONTEXT_TOKENS=2064 TEACHER_TOKENS=0 TAIL_TEACHER_TOKENS=0 DECODE_TOKENS=1 \
DSV41_RUNTIME_BATCHED_DENSE_QMM=1 \
DSV41_RUNTIME_PACKED_EXPERT_BANK=1 DSV41_RUNTIME_COMPACT_EXPERT_BANK=0 \
DSV41_RUNTIME_GROUP_SELECTED_EXPERTS=0 DSV41_RUNTIME_LAYER_FINITE_CHECKS=0 \
DSV41_RUNTIME_RESIDENT_EXPERT_ATLAS=1 DSV41_RUNTIME_ROUTE_DIAGNOSTICS=0 \
DSV41_RUNTIME_INDEX_DIAGNOSTICS=0 DSV41_RUNTIME_CHUNK_ATTENTION=1 \
DSV41_RUNTIME_MLX_CACHE_LIMIT_BYTES=0 DSV41_CONTEXT_EXECUTION=layer_major \
DSV41_RUNTIME_EXPERT_IO_THREADS=4 DSV41_RUNTIME_EXPERT_ASSIGNMENT_CHUNK=128 \
DSV41_RUNTIME_COMPONENT_PROFILE=0 DSV41_CONTEXT_WALL_BUDGET_SECONDS=1200 \
DSV41_CONTEXT_PROJECTED_WALL_LIMIT_SECONDS=0 CACHE_CONDITION=unknown \
RUN_CONDITIONS=resident-shape-bucket-attention-batched-dense-qmm \
bash tools/benchmark/run_context_32k.sh
