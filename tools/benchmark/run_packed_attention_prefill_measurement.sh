...
printf '%s\n' \
 'scope=1 model; resident official layout; one transactional 2063-token sweep + 1 reference decode; oMLX-topology fused packed attention candidate' \
 'resources=allow 5-10 minutes; Unified Memory budget 340 GB; about 289 GB one-time checkpoint reads; checkpoint read-only' \
 'checks=40 model-lifetime banks; zero request bank construction/readback; one fused call per compressed layer/chunk; decomposed shape/QK/AV counts zero' \
...
DSV41_RUNTIME_BATCHED_SPLITK_QK=0 DSV41_RUNTIME_PACKED_CHUNK_ATTENTION=1 \
...
RUN_CONDITIONS=resident-transactional-sweep-packed-fused-attention \
