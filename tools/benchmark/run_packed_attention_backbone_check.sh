...
set -euo pipefail
cd "$(dirname "$0")/../.."
run_dir="artifacts/attention/packed-fused-backbone-$(date +%Y%m%d-%H%M%S)-$$"
mkdir -p "$run_dir"
finish(){
 status=$?
...
printf '%s\n' \
 'scope=40 layers; 2x128 token-serial oracle chunks vs oMLX-topology fused packed attention; official checkpoint' \
 ...
 'dispatch=one 256-thread threadgroup per token; QK/mask/online-softmax/BF16-PV/AV fused; pooled cache decoded in-register; no shape grouping' \
...
shasum -a 256 build-mlx/dsv41-text-backbone-test tests/attention/test_text_backbone.cpp \
 ...
 src/attention/swa_attention.cpp src/attention/packed_chunk_attention.hpp.in \
 metal/attention/packed_chunk_attention.metal metal/attention/steel_attention_header.metal \
...
cmd=(env DSV41_RUNTIME_LAYER_FINITE_CHECKS=0 DSV41_RUNTIME_PACKED_EXPERT_BANK=0 \
 ...
 DSV41_RUNTIME_PACKED_CHUNK_ATTENTION=1 DSV41_CHECK_LAYER_MAJOR_BACKBONE=1 \
...
echo 'Completed; review semantic/state/resource logs before connecting fused packed attention to the 2K production sweep.'
