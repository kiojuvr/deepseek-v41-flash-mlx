...
printf '%s\n' \
  'scope=1 model; resident official layout; one transactional 2063-token layer sweep + 1 reference decode; additive Attention/MoE/post-MoE GPU-completion wall' \
  'resources=allow 5-10 minutes; Unified Memory budget 340 GB; about 289 GB one-time checkpoint reads; checkpoint read-only' \
  'measurement=adds GPU synchronization at component boundaries and therefore perturbs the normal 65.403-second sweep schedule' \
  'checks=40 model-lifetime banks; zero request bank constructions and route/index readbacks; component totals and per-layer distribution require review' \
  'isolation=batched dense QMM remains disabled; no new local Metal kernel is enabled' \
...
DSV41_RUNTIME_COMPONENT_PROFILE=1 \
DSV41_CONTEXT_WALL_BUDGET_SECONDS=1200 DSV41_CONTEXT_PROJECTED_WALL_LIMIT_SECONDS=0 \
CACHE_CONDITION=unknown RUN_CONDITIONS=resident-transactional-layer-sweep-component-profile \
