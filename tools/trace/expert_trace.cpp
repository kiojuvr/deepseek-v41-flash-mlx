...
int main(int argc,char** argv){try{
 ...
 mx::set_default_device(mx::Device::gpu); dsv41::WeightCatalog cat(argv[1],argv[2]); dsv41::MoEReference moe(cat,0); std::vector<mx::array> rows,gates,ups,acts; int occurrences=0;
 for(int t=0;t<int(n);++t){auto row=mx::slice(input,{t,0},{t+1,5120}); auto out=mx::zeros({1,5120},mx::bfloat16),gate=mx::zeros({1,2304},mx::bfloat16),up=gate,act=gate; for(int k=0;k<6;++k) if(selected[t*6+k]==expert_id){auto c=moe.expert_components(row,expert_id,mx::slice(weights,{t,k},{t+1,k+1}));out=c.output;gate=c.gate;up=c.up;act=c.activation;++occurrences;} rows.push_back(out);gates.push_back(gate);ups.push_back(up);acts.push_back(act);}
 std::filesystem::path out=std::filesystem::weakly_canonical(argv[5]); if(std::filesystem::exists(out)) throw std::runtime_error("output must be fresh"); std::filesystem::create_directories(out); auto save=[&](const char* name,const std::vector<mx::array>& v){auto a=mx::concatenate(v,0);mx::eval(a);mx::save((out/name),a);}; save("encoder.layer0.expert_contribution.npy",rows);save("encoder.layer0.expert_gate.npy",gates);save("encoder.layer0.expert_up.npy",ups);save("encoder.layer0.expert_activation.npy",acts);
 std::ofstream f(out/"manifest.json"); f<<nlohmann::json{{"schema_version",1},{"token_ids",m.at("token_ids")},{"expert_id",expert_id},{"occurrences",occurrences},{"arrays",{{{"name","encoder.layer0.expert_contribution"},{"dtype","bfloat16"},{"shape",{n,5120}}},{{"name","encoder.layer0.expert_gate"},{"dtype","bfloat16"},{"shape",{n,2304}}},{{"name","encoder.layer0.expert_up"},{"dtype","bfloat16"},{"shape",{n,2304}}},{{"name","encoder.layer0.expert_activation"},{"dtype","bfloat16"},{"shape",{n,2304}}}}},{"scope","Native routed expert stage diagnostic; no oracle or full-model qualification"}}.dump(2)<<'\n'; std::cout<<"Completed expert trace id="<<expert_id<<" occurrences="<<occurrences<<'\n'; return 0;
