...
int main(int argc,char** argv){try{
 ...
 std::vector<mx::array> raw,corrected,route_weights; std::vector<int> selected; std::vector<float> margins;
 for(int t=0;t<int(n);++t){auto row=mx::slice(input,{t,0},{t+1,5120}); dsv41::RouteTieRecord tie{0,(std::uint64_t)t,0,0,0,0,false}; auto d=gate.diagnose(row,&tie); raw.push_back(d.raw_scores); corrected.push_back(d.corrected_scores); route_weights.push_back(d.route.weights); selected.insert(selected.end(),d.route.ids.begin(),d.route.ids.end()); margins.push_back(tie.sixth_score-tie.seventh_score);}
 ...
 save("encoder.layer0.gate_raw_scores",mx::concatenate(raw,0)); save("encoder.layer0.gate_corrected_scores",mx::concatenate(corrected,0)); save("encoder.layer0.route_weights",mx::concatenate(route_weights,0));
 std::ofstream f(out/"manifest.json"); f<<nlohmann::json{{"schema_version",1},{"token_ids",manifest.at("token_ids")},{"arrays",{{{"name","encoder.layer0.gate_raw_scores"},{"dtype","float32"},{"shape",{n,384}}},{{"name","encoder.layer0.gate_corrected_scores"},{"dtype","float32"},{"shape",{n,384}}},{{"name","encoder.layer0.route_weights"},{"dtype","float32"},{"shape",{n,6}}}}},{"selected_experts",selected},{"boundary_margin_sixth_minus_seventh",margins},{"scope","Native layer0 gate diagnostic; no expert output or full-model qualification"}}.dump(2)<<'\n';
