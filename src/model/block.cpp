...
BlockReference::BlockReference(WeightCatalog& c,int layer,std::shared_ptr<const ResidentExpertAtlas> atlas):layer_(checked_pure_swa_layer(layer)),attn_mix_(c,checked_pure_swa_layer(layer),"attn"),ffn_mix_(c,layer,"ffn"),attention_(c,layer),moe_(c,layer,std::move(atlas)),
 attn_norm_(norm(c,pure_swa_prefix(layer)+".attn_norm.weight")),ffn_norm_(norm(c,pure_swa_prefix(layer)+".ffn_norm.weight")){}
BlockResult BlockReference::forward(const mx::array& h,const mx::array& pre,SwaLayerState& state,std::uint64_t start) const{
 ...
 for(int i=0;i<h.shape(0);++i){
  ...
  trace_record("encoder.layer"+std::to_string(layer_)+".attn_in",attn_in);
  auto attn_out=attention_.forward(attn_in,next,start+i);
  trace_record("encoder.layer"+std::to_string(layer_)+".attn_out",attn_out);
  auto x=hc_post_reference(attn_out,residual,a);
  trace_record("encoder.layer"+std::to_string(layer_)+".post_attn",x);
  auto f=ffn_mix_.mixes(x);residual=x;
  auto ffn_in=rms_norm_reference(hc_pre_reference(x,a.pre),ffn_norm_,1e-20f);
  trace_record("encoder.layer"+std::to_string(layer_)+".ffn_in",ffn_in);
