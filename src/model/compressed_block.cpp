...
CompressedBlockReference::CompressedBlockReference(WeightCatalog& c,int layer,std::shared_ptr<const ResidentExpertAtlas> atlas):layer_(checked_producer_layer(layer)),attn_mix_(c,layer_,"attn"),ffn_mix_(c,layer_,"ffn"),attention_(c,layer_),moe_(c,layer_,std::move(atlas)),
