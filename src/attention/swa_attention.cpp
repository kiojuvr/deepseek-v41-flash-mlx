...
mx::array swa_attention_masked_chunk(const mx::array& q,const mx::array& kv,
 ...
 for(int first=0;first<rows;first+=64){
  ...
  auto keys=mx::where(mx::expand_dims(mask,-1),mx::astype(mx::slice(kv,{0,first,0},{tokens,last,512}),mx::float32),mx::array(0.0f));
  auto scores=mx::multiply(mx::matmul(qf,mx::transpose(keys,{0,2,1})),mx::array(float(std::pow(512.0,-0.5))));
