...
#[derive(Clone)]
struct AppState { model: Arc<String>, backend: Arc<dyn RuntimeBackend> }
...
async fn health(State(state): State<AppState>) -> impl IntoResponse { Json(json!({"status":"ok","runtime":state.backend.name(),"release":false})) }
...
async fn chat(State(state): State<AppState>, Json(req): Json<ChatRequest>) -> (StatusCode, Json<Value>) {
    ...
    match state.backend.complete(&req.messages, req.stream) {
        ...
        Err(backend::BackendError::Unavailable) => (StatusCode::NOT_IMPLEMENTED, Json(json!({"error":{"message":"native runtime backend is not connected","type":"server_error","code":"runtime_unavailable"}}))),
    }
}
...
async fn main() {
    ...
    let app = Router::new().route("/health", get(health)).route("/v1/models", get(models)).route("/v1/chat/completions", post(chat)).with_state(AppState { model: Arc::new(model), backend: Arc::new(UnconnectedBackend) });
